...
export default async function SongPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="material-surface material-cardboard-paper min-h-dvh px-4 py-8 text-foreground sm:px-6 lg:px-[max(1.5rem,calc((100vw-73.75rem)/2))] lg:py-14 print:bg-none print:p-0 print:text-black"><article className="mx-auto flex max-w-6xl flex-col gap-6">
    <Button asChild className="self-start print:hidden" variant="link"><Link href="/songs">← Back to the songbook</Link></Button>
    <Card className="material-surface material-paper-ruled print:bg-white print:text-black print:shadow-none"><CardHeader><div className="flex flex-wrap gap-2"><Badge variant="outline">{song.type ?? "Song"}</Badge><Badge variant={song.verified ? "default" : "secondary"}>{song.verified ? "Reviewed source" : "Source review pending"}</Badge></div><CardTitle className="font-heading text-5xl leading-[0.95] text-balance sm:text-7xl">{song.title}</CardTitle>{song.artist && <CardDescription>{song.artist}</CardDescription>}</CardHeader><CardContent><p className="max-w-3xl text-lg leading-relaxed">Use this on-site teaching sheet for the lyrics, documented music guidance, movement ideas, and curriculum context.</p></CardContent></Card>
