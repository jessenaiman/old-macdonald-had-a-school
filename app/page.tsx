...
function CharacterPerspectiveWall() {
  return (
    <section className="material-surface material-leather relative overflow-visible rounded-3xl p-5 shadow-lg sm:p-8" aria-labelledby="character-perspective-title">
      <header className="mb-6 max-w-2xl">
        <p className="font-body text-xs font-black uppercase tracking-[0.12em] text-brand-paper-foreground/75">The whole room is here</p>
        <h2 className="mt-2 font-section text-[26px] text-brand-paper-foreground" id="character-perspective-title">Sixteen ways to enter the same song</h2>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-8">
        {CHARACTER_KEYS.map((key, index) => {
          ...
          return (
            <article className={`characters-surface relative min-w-0 rounded-xl border border-current/35 p-3 shadow-md ${CHARACTER_NOTE_ROTATIONS[index % CHARACTER_NOTE_ROTATIONS.length]}`} data-character={key} key={key}>
              ...
              <p className="mt-2 truncate text-center text-sm font-bold text-current">{character.name}</p>
...
function HeroBanner() {
  return (
    <section className="relative min-w-0 w-full">
      <div className="material-surface material-leather w-full rounded-3xl p-5 shadow-lg sm:p-8">
        <div className="grid items-center gap-8 lg:grid-cols-[1.05fr_auto_0.95fr]">
          <div className="flex min-w-0 flex-col gap-6">
            <h1 className="break-words font-heading text-[clamp(1.9rem,4vw,2.75rem)] leading-[1.05] text-brand-paper-foreground">
              Where familiar songs become{" "}
              <em className="not-italic text-[color-mix(in_oklab,var(--rose-warm)_35%,var(--brand-paper-foreground))]">new places</em> to learn.
            </h1>
