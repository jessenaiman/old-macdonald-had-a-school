  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}>
        <div className={styles.breadcrumb}>Curriculum-organized starting points</div>
      ...
      <section className={styles.list} aria-label="Lesson topics">
        {filtered.map((lesson) => (
          <Link
            href={lessonHref(lesson)}
            className={`${styles.card} [content-visibility:auto] [contain-intrinsic-size:auto_240px] print:[content-visibility:visible] print:[contain-intrinsic-size:none]`}
            key={lesson.slug}
          >
            <div><span>{lesson.subject}</span><strong>{lesson.category}</strong></div>
            <h2>{lesson.title}</h2>
            <p>{lesson.summary}</p>
            <div className={styles.action}>{lesson.grade}<strong>Open lesson →</strong></div>
          </Link>
        ))}
        {filtered.length === 0 && (
          <p className={styles.empty}>No topics match this filter yet.</p>
        )}
      </section>
    </div>
  );
