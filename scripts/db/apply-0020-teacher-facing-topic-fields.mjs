...
const migrationId = "20260813_005_add_teacher_facing_topic_fields";
...
const migrationPath = path.join(root, "src", "db", "migrations-sqlite", "0020_add_teacher_facing_topic_fields.sql");
...
try {
  ...
  console.log(JSON.stringify({ status: "applied", migrationId, database: databasePath, backup: backupPath, integrity, foreignKeyErrors: foreignKeys.length }, null, 2));
} finally {
  db.close();
}
