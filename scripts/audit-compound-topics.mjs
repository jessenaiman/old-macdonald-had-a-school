...
const compound = db.prepare(`
  SELECT id, lesson_topic, skill_statement, subject_id 
  FROM TOPICS 
  WHERE lesson_topic LIKE '% & %' 
     OR lesson_topic LIKE '% / %' 
     OR lesson_topic LIKE '% and %'
...
compound.forEach(t => {
  console.log('\n[' + t.id + '] ' + t.lesson_topic);
  console.log('  Skill:', (t.skill_statement || 'NULL').slice(0, 120));
});
...
compound.forEach(t => {
  ...
  console.log('[' + t.id + '] ' + t.lesson_topic.slice(0, 40) + ': grades=' + grades.n + ' materials=' + materials.n + ' standards=' + standards.n + ' tags=' + tags.n + ' pacing=' + pacing.n);
});
