    ...
    <CardContent>
     <form onSubmit={submitSearch} role="search" aria-busy={loading}>
      <FieldGroup className="gap-4 lg:grid lg:grid-cols-[minmax(0,1fr)_12rem_12rem_auto] lg:items-end">
       ...
       {lockedGrade ? null : <Field><FieldLabel htmlFor="search-grade">Grade</FieldLabel><NativeSelect id="search-grade" name="grade" className="min-h-11" value={grade} onChange={(event) => setGrade(event.target.value)}>{GRADE_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</NativeSelect></Field>}
       <Field><FieldLabel htmlFor={lockedGrade ? `kind-${lockedGrade}` : "search-kind"}>Resource type</FieldLabel><NativeSelect id={lockedGrade ? `kind-${lockedGrade}` : "search-kind"} name="kind" className="min-h-11" value={kind} onChange={(event) => setKind(event.target.value)}><option value="">All resources</option><option value="song">Songs</option><option value="knowledge">Knowledge</option></NativeSelect></Field>
       <Field className="lg:w-auto"><Button className="min-h-11" type="submit" disabled={loading}>{loading ? "Searching…" : "Search"}</Button></Field>
      </FieldGroup>
     </form>
    </CardContent>
   </Card>
