...
export function GradeWelcomeControl({ config, summary, primaryHref, onBrowse, onPreview }: { config: GradeInteractionConfig; summary: string; primaryHref?: string; onBrowse?: () => void; onPreview?: () => void }) {
  return <ResponsiveFeatureSplit ratio="primary" className={`${styles.welcome} ${config.variant === "daycare" ? styles.daycareWelcome : ""}`}>
      <div className={styles.welcomeCopy}><span className={styles.eyebrow}>{config.eyebrow}</span><h1>{config.headline} <em>{config.accentHeadline}</em></h1><p>{summary}</p><div className={styles.actions}>{primaryHref ? <Link href={primaryHref} className={styles.primaryAction}>Build this lesson</Link> : onPreview ? <Button variant="ghost" type="button" className={styles.primaryAction} onClick={onPreview}>Preview the story</Button> : null}<Button variant="ghost" type="button" className={styles.paperAction} onClick={onBrowse}>Browse learning paths</Button></div></div>
...
function PersistentTeacherQuote({ config }: { config: GradeInteractionConfig }) {
  return (
    <aside className={styles.teacherQuote} aria-label={`A note from ${config.leadName}`}>
      <Card className={`${styles.teacherCard} ${config.teacherClass} character-surface`}>
        ...
        <CardContent className={`${styles.teacherCardContent} lg:!pr-[44%]`}>
          ...
          <Image src={config.leadImage} width={220} height={220} alt={config.leadName} className={`${styles.teacherImage} lg:!w-[44%]`} priority />
        </CardContent>
      </Card>
    </aside>
  );
