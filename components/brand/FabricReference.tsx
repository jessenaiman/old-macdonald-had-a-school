    ...
    {characters.map((character) => {
      ...
      return <Card key={character}>
        ...
        <CardContent className="grid gap-3">
          <div className={cn(`cast-${character}`, "material-surface", "material-felt", specimenClass)} aria-label={`${identity.name} felt specimen`} />
...
export function FabricReference() {
  return <>
    <div className="mt-7 grid gap-4 md:grid-cols-2">
      <Card><CardHeader><CardTitle>Identity felt</CardTitle><CardDescription>Soft, stitched, and locally owned</CardDescription></CardHeader><CardContent className="grid gap-3"><div className={cn("cast-old-macdonald", "material-surface", "material-felt", specimenClass)} aria-label="Old MacDonald felt specimen" /><p className="m-0 font-body text-base font-semibold leading-[1.7]">Production consumer: the shared TeacherNote and existing identity patches. The paper Card remains the readable block.</p><code>cast-old-macdonald material-surface material-felt</code></CardContent></Card>
      <Card><CardHeader><CardTitle>Supporting woven cloth</CardTitle><CardDescription>An attached neutral classroom textile</CardDescription></CardHeader><CardContent className="grid gap-3"><div className={cn("material-surface", "material-woven-fabric", specimenClass)} aria-label="Woven fabric specimen" /><p className="m-0 font-body text-base font-semibold leading-[1.7]">Registered inventory sample. No production control currently owns this treatment.</p></CardContent></Card>
    </div>
