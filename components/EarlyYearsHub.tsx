...
export function EarlyYearsHub({ grade, title, tagline, lead, topics }: { grade: EarlyYearsGradeKey; title: string; tagline: string; lead: { patch: CharacterKey; name: string }; tier: EarlyYearsTier; topics: EarlyYearsTopic[] }) {
  ...
  return <div className="min-w-0" data-grade={grade}>
    <GradePageTemplate grade={title} age={grade === "daycare" ? "Ages 0–2" : grade === "pre-school" ? "Ages 3–4" : "Ages 4–6"} leadName={lead.name} leadImage={leadImage} leadQuote={leadQuote} headline={headline} accentHeadline={accentHeadline} summary={tagline} activeIndex={active} onSelect={setActive} onPreview={topicImage ? () => setPreview(true) : undefined} items={topics.map((item, index) => ({ title: item.title, kicker: index === 0 ? "Story circle" : index === 1 ? "Mix & measure" : index === 2 ? "Explore together" : "Make & explain", summary: item.focus, href: `/grade/${item.grade}/${item.slug}`, icon: lessonIcon(item.title, item.focus) }))} />
