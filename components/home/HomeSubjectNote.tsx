...
export function HomeSubjectNote({ title, href, iconClass, teacherReason, highlights, guideCharacter, fastenerClass }: HomeSubjectNoteProps) {
  return (
    <Card className={`characters-${guideCharacter} material-surface material-cardboard-paper relative h-full min-w-0 gap-0 overflow-visible border-border py-0 text-[var(--theme-ink)] transition-transform hover:-translate-y-0.5`}>
      ...
      <CardHeader className="flex min-h-28 flex-row items-center gap-3 border-b border-border p-5">
        ...
        <div className="min-w-0">
          <CardTitle className="font-heading text-2xl text-[var(--theme-ink)]"><Link className="focus-visible:outline-3 focus-visible:outline-ring focus-visible:outline-offset-4" href={href}>{title}</Link></CardTitle>
          <p className="mt-2 text-sm leading-5 text-[var(--brand-paper-muted)]">{teacherReason}</p>
        </div>
      </CardHeader>
      <CardContent className="flex-1 p-0">
        <nav className="grid" aria-label={`${title} lesson topics`}>
          {highlights.map((highlight) => (
            <Link key={highlight} className="border-b border-border px-5 py-3 text-sm font-semibold text-[var(--theme-ink)] last:border-b-0 hover:bg-gold-bright/10 focus-visible:bg-gold-bright/10 focus-visible:outline-3 focus-visible:outline-ring focus-visible:outline-offset-[-3px]" href={`/search?q=${encodeURIComponent(`${title} ${highlight}`)}`}>
      ...
      <CardFooter className="min-h-16 border-t border-border px-5 py-4">
        <Link className="text-sm font-bold text-[var(--theme-ink)] underline underline-offset-4" href={href}>View all {title.toLowerCase()} lessons <span aria-hidden="true">→</span></Link>
      </CardFooter>
    </Card>
  );
}
