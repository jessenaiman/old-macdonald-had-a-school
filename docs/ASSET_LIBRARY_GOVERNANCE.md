...
| --- | --- | --- | --- | --- | --- | --- |
| PAM-NOTE-001 | Generate review family and separate transparent derivatives | New original image-generation source, visually guided by `qa/homepage-concepts/revised-option-1.png` and the approved construction-paper material family | `public/design-assets/classroom-paper-notes-v1-review/classroom-paper-notes-sheet-v01.png` and `individual-notes/01-torn-note.png` through `06-straight-note.png` | Add responsive blank teacher-note silhouettes that can hold semantic HTML text, approved icons, and independently positioned fasteners; no text, icons, or fasteners baked into the raster assets | New review directory is removable; no existing asset overwritten | APPROVED by user on 2026-08-13 |
